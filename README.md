...
1. **Liste des équipes** : alignée sur le document officiel « Fonctionnement des équipes » — Nav Team, Doc Team, Event Team, Digital Team, Training Department, vACC Directors, dans cet ordre (`src/config/departments.ts`). Le pôle Membership n'y figure pas comme équipe ; si une catégorie « Membership » est utile (journal des sollicitations notamment), elle peut être ajoutée d'une ligne.
