...
2. **Thème** : identité éditoriale « magazine » — blanc chaud, rouge éditorial et bleu paon, titres en **Fraunces** (display) et texte en **Newsreader** (serif de lecture) auto-hébergés via Fontsource (le site reste statique), avec lettrines, filets et intertitres. Centralisé dans `src/index.css` (import des polices dans `src/main.tsx`), contrastes vérifiés WCAG AA.
